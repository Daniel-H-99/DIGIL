...
class ClsDataset(Dataset):
    def __init__(self, root_dir, split='train'):
        ...
        self.split = split
        self.path = os.path.join(self.root_dir, f'cls_{self.split}.pt')
        self.data = torch.load(self.path)  # N x 2

    def __len__(self):
        return len(self.data)

    def __getitem__(self, idx):
        return {'food': self.data[idx][0], 'ans': self.data[idx][1]}
...
class CompDataset(Dataset):
    def __init__(self, root_dir, split='train'):
        ...
        self.root_dir = root_dir
        self.split = split
        self.path = os.path.join(self.root_dir, f'comp_{self.split}.pt')
        self.data = torch.load(self.path)  # N x 2
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            target_idx = np.random.randint(len(item))
            target = item[1][target_idx]
            ing = item[1][:target_idx] + item[1][target_idx + 1:]
            food = item[0]
        else:
            target = item[2]
            ing = item[1]
        ...
        return {'food': food, 'ing': ing, 'ans': target}

class cuisineDataset(Dataset):
    def __init__(self, path, cuisine_idx, split=1.0):
        super(cuisineDataset, self).__init__()
        self.path = path
        self.data = []
        self.split = split
        ...
        with open(path) as infile:
            for i, line in enumerate(infile):
                line=line.strip().split(",")
                # self.data.append((i, list(map(lambda x: int(x), line[:-1])), cuisine_idx.index(line[-1])))
                self.data.append((i, -1, cuisine_idx.index(line[-1])))
        ...
        if self.split < 0:
            self.data = self.data[int(split * len(self.data)):]
        else:
            self.data = self.data[:int(split * len(self.data))]
...
class wikiDataLoader(object):
    ...
    def __init__(self, filename, batch_size, opt, user_real_dict, user_fake_dict, item_real_dict, item_fake_dict, evaluation):
        ...
        with open(filename) as infile:
            ...
            for line in infile:
                line=line.strip().split("\t")
                data.append([int(line[0]),int(line[1]),1])
                # data.append([int(line[0]),int(line[1]),int(line[2])])
                if int(line[0]) not in self.ma.keys():
                    self.ma[int(line[0])] = set()
                self.ma[int(line[0])].add(int(line[1]))
