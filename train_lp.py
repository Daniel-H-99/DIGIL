...
from model.trainer import DGITrainer, DGILTrainer
from utils.loader import DataLoader,wikiDataLoader, cuisineDataset, ClsDataset, CompDataset
...
parser.add_argument('--lr_cls', type=float, default=0.001, help='Applies to sgd and adagrad.')
parser.add_argument('--lr_comp', type=float, default=0.001, help='Applies to sgd and adagrad.')
...
# parser.add_argument('--num_cuisine', type=bool, default=True, help='wiki-dataset')
...
train_batch = wikiDataLoader(opt['data_dir'] + 'edges.txt', opt['batch_size'], opt,
...
def cmp_collate_fn(samples):
    ...
    for i, sample in enumerate(samples):
        ing_r.extent([i] * len(sample['ing']))
        ing_c.extend(sample['ing'])
    ans = [sample['ans'] for sample in samples]
...
for epoch in range(1, opt['num_epoch'] + 1):
    ...
    for i, batch in enumerate(tqdm(train_batch)):
        global_step += 1
        loss_recon = trainer.reconstruct(UV, VU, adj,corruption_UV, corruption_VU, fake_adj, user_feature, item_feature, batch)  # [ [user_list], [item_list], [neg_item_list] ]
        train_loss += loss_recon
        cnt += 1
        if cnt == 100:
            break

    #     loss_cls = trainer.classify(batch[0].cuda(), batch[2].cuda())
    #     train_loss += loss_cls

    for i, batch in enumerate(tqdm(cls_train_loader)):
        # global_step += 1
        # loss = 0
    ...
    trainer.epoch_cls_loss = []
    trainer.epoch_comp_loss = []
    ...
    num_correct = 0
    num_ques = 0
    for i, batch in enumerate(tqdm(cls_dev_loader)):
        pred_labels = trainer.classify_pred(batch)
        num_correct += (pred_labels.detach().cpu() == batch['ans']).sum()
        num_ques += len(batch['ans'])
    
    cls_acc = num_correct / num_ques

    num_correct = 0
    num_ques = 0
    for i, batch in enumerate(tqdm(comp_dev_loader)):
        pred_labels = trainer.classify_pred(batch)
        num_correct += (pred_labels.detach().cpu() == batch['ans']).sum()
        num_ques += len(batch['ans'])
    
    comp_acc = num_correct / num_ques

    train_loss = train_loss / train_batch.num_examples * opt['batch_size']  # avg loss per batch
    
    print("epoch {}: train_loss = {:.6f}, cls_acc = {:.6f}, comp_acc = {:.6f}".format(epoch, \
                                                                                    train_loss, cls_acc, comp_acc))
